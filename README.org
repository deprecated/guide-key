...
- *guide-key/popup-buffer-hook*: A hook that is run every time the guide buffer is created.  You can use this for fine-tuning the appearance of the buffer, for instance changing the font size or removing the mode-line: 
#+BEGIN_SRC lisp
